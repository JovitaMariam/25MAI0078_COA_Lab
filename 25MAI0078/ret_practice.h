...
void fn_rer_practice1_srl() {
	...
	start = omp_get_wtime();
	printf("Hello Jovita serial = %d \n", omp_get_thread_num());
	end = omp_get_wtime();
	printf("Execution time for the serial of Hello Jovita is : %f \n", end - start);
	ETserial = end - start;
}
...
void fn_rer_practice1_prl() {
	...
	start = omp_get_wtime();
#pragma omp parallel
	printf("Hello Jovita parallel = %d \n", omp_get_thread_num());
	end = omp_get_wtime();
	printf("Execution time for the parallel of Hello Jovita is : %f \n", end - start);
